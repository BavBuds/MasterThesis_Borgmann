...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Coverage Analysis Pipeline')
    parser.add_argument('-s', '--samples', type=str, default=None,
                        help='Comma-separated list of default samples to process (HT,HT2,BB)')
    parser.add_argument('--custom', type=str, action='append',
                        help='Add custom sample in format: sample_id:assembly_path:reads_type:read1_path[:read2_path] '
                             'where reads_type is ONT or ILLUMINA')
    parser.add_argument('--ont', type=str, default=None,
                        help='Comma-separated list of sample IDs that are ONT data')
    parser.add_argument('-t', '--threads', type=int, default=30,
                        help='Number of CPU threads to use for processing')
    parser.add_argument('-o', '--output', type=str, 
                        default="/data/proj2/home/students/m.borgmann/Master_thesis/data/processed/Popgen_analysis/dataprep/5mb_subset",
...
def main():
    ...
    default_samples = {
        "HT": {
            "assembly": os.path.join(BASE_DIR, "Input_fastas", "HT", "HT_assembly_5mb_subset.fasta"),
        ...
        "HT2": {
            "assembly": os.path.join(BASE_DIR, "Input_fastas", "HT2", "HT2_assembly_5mb_subset.fasta"),
        ...
        "BB": {
            "assembly": os.path.join(BASE_DIR, "Input_fastas", "BB", "BB_assembly_5mb_subset.fasta"),
    ...
    if args.samples:
        # User specified default samples to use
        selected_defaults = [s.strip() for s in args.samples.split(',')]
        for sample_id in selected_defaults:
            if sample_id in default_samples:
                samples[sample_id] = default_samples[sample_id]
                logger.info(f"Added default sample: {sample_id}")
            else:
                logger.warning(f"Default sample '{sample_id}' not found. Available: {', '.join(default_samples.keys())}")
    elif not args.custom:
        # No specific samples or custom samples, prompt user
        logger.info("No samples specified. Choose from default samples:")
        for idx, key in enumerate(default_samples.keys(), start=1):
            logger.info(f"{idx}: {key}")
        
        selected_input = input("Enter default sample numbers (comma separated) to process, or 'all': ")
        if selected_input.lower().strip() == 'all':
            samples = default_samples.copy()
            logger.info("Processing all default samples")
        else:
            selected_indices = [int(i.strip()) for i in selected_input.split(',') if i.strip().isdigit()]
            all_keys = list(default_samples.keys())
            for idx in selected_indices:
                if 1 <= idx <= len(all_keys):
                    sample_id = all_keys[idx-1]
                    samples[sample_id] = default_samples[sample_id]
                    logger.info(f"Added default sample: {sample_id}")

    # Add custom samples if provided
    if args.custom:
        for custom_def in args.custom:
            parts = custom_def.split(':')
            if len(parts) < 4:
                logger.error(f"Invalid custom sample format: {custom_def}")
                logger.error("Format should be: sample_id:assembly_path:reads_type:read1_path[:read2_path]")
                continue
                
            sample_id = parts[0]
            assembly_path = parts[1]
            reads_type = parts[2].upper()
            
            if reads_type not in ['ONT', 'ILLUMINA']:
                logger.error(f"Invalid reads type '{reads_type}'. Must be ONT or ILLUMINA")
                continue
                
            if reads_type == 'ONT':
                if len(parts) != 4:
                    logger.error(f"ONT sample needs exactly 4 parts: {custom_def}")
                    continue
                ...
                if len(parts) != 5:
                    logger.error(f"ILLUMINA sample needs exactly 5 parts: {custom_def}")
                    continue
            ...
            logger.info(f"Added custom sample: {sample_id} ({reads_type})")
    ...
    # List samples to be processed
    logger.info("\nSamples to process:")
    for idx, key in enumerate(samples.keys(), start=1):
        logger.info(f"{idx}: {key} ({samples[key]['data_type']})")

    # Determine which samples are ONT
    ont_datasets = set()
    
    if args.ont:
        # User specified ONT samples in command line
        ont_datasets = set(s.strip() for s in args.ont.split(','))
    else:
        # Look for ONT in data_type and/or ask user
        for sample_id, sample_info in samples.items():
            if sample_info['data_type'] == 'ONT':
                ont_datasets.add(sample_id)
        
        # Double-check with user if needed
        if not ont_datasets:
            ont_input = input("Enter dataset numbers (comma separated) that are ONT, or press Enter if none: ")
            if ont_input.strip():
                numbers = [int(num.strip()) for num in ont_input.split(",") if num.strip().isdigit()]
    ...
    # Print a summary of the created filtered VCF files
    logger.info("\n==========================")
    logger.info("📄 Filtered VCF Files:")
    logger.info("==========================")
    for sample in samples:
        if os.path.exists(filtered_vcf_files[sample]):
            size_mb = os.path.getsize(filtered_vcf_files[sample]) / (1024 * 1024)
            logger.info(f"✅ {sample}: {filtered_vcf_files[sample]} ({size_mb:.2f} MB)")
        else:
            logger.info(f"❌ {sample}: Not created")
    
    # Provide command for PSMC pipeline
    logger.info("\n==========================")
    logger.info("🔄 Next Step: PSMC Pipeline")
    logger.info("==========================")
    psmc_cmd = (
        f"python psmc_pipeline.py \\\n"
        f"  -i {INTERMEDIATE_DIR} \\\n" 
        f"  -r {BASE_DIR} \\\n"
        f"  -o /data/proj2/home/students/m.borgmann/Master_thesis/data/processed/Popgen_analysis/psmc_results \\\n"
        f"  -s {','.join(samples.keys())} \\\n"
        f"  -t {NUM_THREADS} \\\n"
        f"  -b 100"
    )
    logger.info("You can now run the PSMC pipeline with this command:")
    logger.info(psmc_cmd)
